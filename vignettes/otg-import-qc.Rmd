...
**At this point in the workflow, it is probably best that someone very familiar with the OTG data (e.g., field technician, field coordinator) should intervene, review the QC results, and attempt to resolve them. Ideally, notes should also be taken about those QC errors that can't be resolved, and for those that can be resolved, identify how the issue was resolved. Similar to above, it is best practice to not modify the "raw" data (i.e., don't make changes to the data in the "/1_formatted_csvs/" directory). Instead, it is a good idea to make another copy of the data (I use "/2_qcd_csvs/") and resolve any identified issues in that data. In this case, someone could then fairly easily identify any changes that were made to the data during the QC process.** In addition, by documenting changes that have been made to the data, we can identify common errors and perhaps add data validation during the data collection process to prevent those errors in the future.
...
# write QC results to .csv
write_csv(qc_final,
          paste0(nas_prefix, "/data/habitat/DASH/OTG/2_qcd_csvs/final_qc.csv"))

# append today's date
write_csv(init_qc,
          paste0(nas_prefix,
                 "/data/habitat/DASH/OTG/2_qcd_csvs/final_qc_",
                 format(Sys.Date(), format = "%Y%m%d"),
                 ".csv"))
